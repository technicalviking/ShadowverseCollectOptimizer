...
{
    public class CraftAllFullCollection
    {
            ...
            craftRarityStrategies.ForEach(crs =>
            {
                craftDupeStrategies.ForEach(cds =>
                {
                    colCompStrats.ForEach(ccs =>
                    {
                        ...
                        CraftAllRoundRobinPlayer carrp = new CraftAllRoundRobinPlayer(ccs, crs, cds);
                        players.Add(carrp);
                        runs.Add(carrp.name, new int[MAXRUNS]);
                    });
                });
            });
        ...
        private void RunTests()
        {
            ...
            for (var i = 0; i < MAXRUNS; i++)
            {
                ...
                {
                    p.InitPlayer();
                    p.AddCollection(new DarknessBreakdown());
                    p.AddCollection(new BahamutBreakdown());
                    p.AddCollection(new TempestBreakdown());
